...
front=re.compile(r'^---\n(.*?)\n---\n',re.S)
idpat=re.compile(r'b(?:A+d{3}|[A-G]-d{3}[a-z]?)b')
...
errors=[]; ids={}; names={}; canonical_files=[]; artifact_files=[]
for p in files:
 d=parse(p)
 if not d: errors.append(f'MISSING_FRONT_MATTER:{p.relative_to(ROOT)}'); continue
 if d.get('namespace') == 'NODE_ARTIFACT':
  artifact_files.append(p)
  for key in ['artifact_id','parent_node_id','title','namespace','lifecycle','metadata_standard']:
   if key not in d: errors.append(f'MISSING_ARTIFACT_FIELD:{p.relative_to(ROOT)}:{key}')
...
report={'pass':not errors,'node_file_count':len(canonical_files),'unique_node_ids':len(ids),'artifact_file_count':len(artifact_files),'gate_counts':{},'lifecycle_counts':{},'unresolved_ids':unresolved,'errors':errors}
from collections import Counter
report['gate_counts']=dict(Counter(parse(p).get('gate') for p in canonical_files))
report['lifecycle_counts']=dict(Counter(parse(p).get('lifecycle') for p in canonical_files))
