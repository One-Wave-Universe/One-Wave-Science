...
6. remote MCP client can call `terminal_pwd`;
7. remote MCP client can run `git status --short --branch`;
8. SSH works independently;
9. `git fetch origin` works on the Jetson;
10. a disposable task branch can be pushed or published through an authorized GitHub integration;
11. CERN record metadata can be retrieved without downloading the full dataset;
12. GWOSC `/api/v2/runs` can be queried without authentication;
13. one small CERN or GWOSC sample can be written to an approved data directory;
14. provenance is recorded with the test result.
