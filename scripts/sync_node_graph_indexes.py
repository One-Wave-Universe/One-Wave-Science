#!/usr/bin/env python3
"""Synchronize canonical node discovery surfaces.
...
NODE_ARTIFACT documents are intentionally excluded from the master node list.
"""
...
def canonical_nodes() -> dict[str, tuple[str, str, str, str]]:
    """Return node_id -> (name, gate, classification, source path)."""
    nodes: dict[str, tuple[str, str, str, str]] = {}
    for directory in NODE_DIRS:
        for path in sorted(directory.glob("*.md")):
            meta = frontmatter(path)
            if meta.get("namespace") != "NODE" or not meta.get("node_id"):
                continue
            node_id = meta["node_id"]
            nodes[node_id] = (
                meta.get("canonical_name", path.stem),
                meta.get("gate", "BROWN"),
                meta.get("classification", "Canonical node"),
...
def strip_supplement(text: str) -> str:
    if SUPPLEMENT_START not in text and SUPPLEMENT_END not in text:
        return text.rstrip() + "\n"
    if text.count(SUPPLEMENT_START) != 1 or text.count(SUPPLEMENT_END) != 1:
        raise RuntimeError("master index has malformed auto-node registry markers")
...
def build_supplement(base_text: str) -> str:
    ...
    for node_id, (name, gate, classification, source) in sorted(canonical_nodes().items()):
        ...
        if node_id in represented:
            continue
        missing.append((node_id, name, gate, classification, source))
    ...
    if missing:
        for node_id, name, gate, classification, source in missing:
            definition = f"{clean_cell(classification)}. Source: `{source}`."
            lines.append(f"| {node_id} | {clean_cell(name)} | {definition} | {clean_cell(gate)} |")
    else:
        lines.append("| — | No unplaced canonical nodes | Every canonical node is represented in the curated tables above. | — |")
    lines.extend([SUPPLEMENT_END, ""])
    return "\n".join(lines)
...
def sync_master(text: str) -> str:
    ...
    base = text.rstrip() + "\n\n"
    return base + build_supplement(base)
