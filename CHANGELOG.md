...
# Updated 44 — Keplerian Limit Derivation

**Renumbered from Updated 42** during reconciliation with PR #6's parallel branch, which independently used 42/43 for unrelated content (both branches forked from `main` at 41). See `UPDATED_44_KEPLERIAN_LIMIT_DERIVATION.md`'s own note.
...
- Reduced the far-field limit to Newton's inverse-square law in closed form — originally stated as `G_eff = 1/(4*pi*alpha)`, corrected below to `G_eff = alpha/(4*pi*a)` — closing Book 1 Ch12's own "sketch — derivation deferred" status on that result.
- Proved a monopole far-field equivalence for this framework from first principles (corrected below — an earlier version of this entry called this the full shell theorem, which overstates it), giving real but conditional support (exact under spherical symmetry; monopole-exact and dipole-vanishing-about-centroid for any shape) for what Updated 38-41 assume throughout. Also derived that naive linear superposition of multiple such fields reproduces "summing independent gravitational wells to infinity" — the exact picture Updated 38's finite-wake hypothesis was proposed to replace, not a justification of it; corrected an earlier draft that wrongly claimed superposition supported that architecture.
- **Correction (external review, applied after this entry was first written):** the G_eff formula in this update's derivation was wrong — an earlier version mixed two different field equations during source-matching and stated `G_eff=1/(4*pi*alpha)`; the corrected, verified formula is `G_eff=alpha/(4*pi*a)`, using A-106's own field-generation coefficient `a` alongside A-105's response coefficient `alpha` rather than substituting one for the other. The "shell theorem" claim above was also corrected to the weaker, honest monopole-equivalence statement. See `Internal_Proofs/08_Keplerian_Limit_Derivation.md`'s own correction notice for the full derivation of both fixes. The Kepler-law derivation itself is unaffected by either correction.
