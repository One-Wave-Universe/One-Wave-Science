...
`One_Wave_Bench/Virtual_Breadboard/` (repo root, merged to `main`) is a real modified-nodal-analysis
...
- Interactive, in-browser: open `One_Wave_Bench/Virtual_Breadboard/index.html` (or
...
- Headless, for an automated loop (e.g. a Python driver on other hardware):
  `node One_Wave_Bench/Virtual_Breadboard/simulate.js` reads a circuit spec as JSON on
  ...
  beyond Node itself.
- Regression tests: `node One_Wave_Bench/Virtual_Breadboard/test/circuit.test.js` — see
...
- Full usage docs: `One_Wave_Bench/Virtual_Breadboard/README.md`.
