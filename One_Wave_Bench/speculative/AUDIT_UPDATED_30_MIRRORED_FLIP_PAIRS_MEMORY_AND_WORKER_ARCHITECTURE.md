...
Today’s rabbit-hop work is already represented in `One_Wave_Bench/speculative/ARCHITECTURE_RABBIT_HOPPING_SCALE_TRANSLATOR.md` and `One_Wave_Bench/speculative/AI_Readable_Packs/G-721_Mirrored_Alphabet_Rabbit_Hop.json`.
...
Today’s canonical memory architecture is in `One_Wave_Bench/speculative/ARCHITECTURE_MEMORY_REBUILD_CONSTELLATION.md`.
...
8. Run the minimum constellation-memory rebuild test already defined in `One_Wave_Bench/speculative/ARCHITECTURE_MEMORY_REBUILD_CONSTELLATION.md`.
