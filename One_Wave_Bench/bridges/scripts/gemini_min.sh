...
usage() {
  cat <<'EOF'
Usage:
  One_Wave_Bench/bridges/scripts/gemini_min.sh review TASK.md
  One_Wave_Bench/bridges/scripts/gemini_min.sh code TASK.md
  One_Wave_Bench/bridges/scripts/gemini_min.sh ask "short question"
  GEMINI_ALLOW_PRO=1 One_Wave_Bench/bridges/scripts/gemini_min.sh deep TASK.md
...
if ! command -v gemini >/dev/null 2>&1; then
  if [[ -x "$HOME/.local/bin/gemini" ]]; then
    ...
    fail "gemini not found; run One_Wave_Bench/bridges/scripts/install_gemini_jetson.sh first"
  fi
fi
