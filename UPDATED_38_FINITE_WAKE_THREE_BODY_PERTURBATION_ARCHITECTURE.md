...
`Internal_Proofs/08_Keplerian_Limit_Derivation.md` derives, from A-105+A-106 rather than assuming it, the leading-order single-source limit that `grad(Phi_i)` reduces to for one bounded source in isolation: an exact inverse-square response, plus a monopole far-field equivalence (corrected from an overstated "shell theorem" claim after external review — exact under spherical symmetry only; real but weaker support for treating one extended body, Sun/Jupiter/etc., as approximately point-like at its own centroid, not an unconditional exact statement).

**It does not justify, and should not be read as justifying, the finite-wake hypothesis this document is built around.** That derivation's field has no cutoff at any distance — it is a plain `1/r` potential extending to infinity, for every source, unconditionally (Internal_Proofs/08 §4.3). Naively summing several of those is precisely "summing independent gravitational wells to infinity," the picture this document's Purpose section explicitly says the finite-wake hypothesis replaces. An earlier version of this note claimed the derived superposition supported the additive relational structure above; that was wrong and is corrected here. The finite-wake assimilation criterion, the non-tautological closure invariant, and the Mercury residual this update calls for remain exactly as open as stated above — and the tension between "long-range inverse-square gravity" (now derived) and "finite-wake cutoff" (still just hypothesized) is a real open question this closure surfaces rather than resolves.
