...
`https://github.com/One-Wave-Universe/Builds/tree/main/runtime/bench/schema/experiment_protocol.json` — defines an experiment
...
- `https://github.com/One-Wave-Universe/Builds/tree/main/runtime/bench/engine/run_experiment.py` imports the **canonical** D-413
...
- `https://github.com/One-Wave-Universe/Builds/tree/main/runtime/bench/engine/build_manifest.py` scans `runs/` and writes
  `runs/manifest.json`, which the bench UI reads.

### 3. Shared bench UI
`https://github.com/One-Wave-Universe/Builds/tree/main/runtime/bench` — one page both a human and an AI use:
...
`sims/bench/fixed_sim.html` — locked-parameter, reproducible player selected
