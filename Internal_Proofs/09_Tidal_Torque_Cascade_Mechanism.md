...
## 1. The tidal tensor, derived from the candidate derived field (not asserted)

**Correction (external review): `Internal_Proofs/08`'s field is not "already-verified."** That draft had two real mathematical errors (a source-matching mixup giving the wrong `G_eff`, and an overstated shell theorem) caught and fixed after this document was first written. The tidal tensor below is unaffected in *form* — it depends on the `1/r` structure of the far-field solution, not on the specific value of `G_eff` — but every reference in this document to that field as "already-verified" should be read as **candidate derived field**, matching the corrected status in `Internal_Proofs/08` itself.
...
| galaxy cluster → individual galaxy | `M~3×10¹⁴ M☉` at `R~10⁷ ly` | galaxy, `M~2×10¹¹ M☉`, `r~10⁵ ly` | **~10⁻²·⁸** (a few tenths of a %, in the same broad order of magnitude as real spin parameters `λ~0.05` — flagged below as coincidental proximity, not a validation) |
...
The claim "galaxy clusters caught in the Great Attractor is where galactic-on-down rotation begins" needs one precise correction: **a direct, skip-level pull is computed and ruled out (§3's table); what replaces it is at most a candidate relay interpretation, not a proven one.** §3 shows that adjacent nested levels (Attractor↔cluster↔galaxy) have non-negligible tidal ratios while distant-level pairs (galaxy↔star system) do not — that is a real, computed pattern. Reading this as evidence that angular momentum is *actually transmitted* successively down the hierarchy, level by level, is a further interpretive step this document has not earned: similar tidal-ratio magnitudes between adjacent levels do not, by themselves, prove a causal handoff chain, as opposed to each level's rotation arising largely from its own local formation dynamics while merely also sitting in a tidal environment of comparable strength. The "relay" language below should be read as the natural candidate reading of the computed numbers, not as established. This also requires separating two genuinely different real effects that "caught in the Great Attractor" can blur together, and B-226's own P/Pa/F vocabulary keeps them apart correctly:
...
- The tidal-torque mechanism is now derived directly from this repository's own candidate derived field (`Internal_Proofs/08` — corrected, not "already-verified"; see §1), not imported as an unconnected outside fact — closing a real gap between the Kepler-limit work and this session's rotation question.
...
- The correct, weaker, but real claim — non-negligible tidal ratios between comparably-sized nested levels (Attractor↔cluster↔galaxy), each individually small but dynamically real — is computed. Reading this as a "relay" (successive transmission) is a candidate interpretation of that computed pattern, not proven by it (§4, corrected). The tidal-ratio magnitude landing near real galaxy spin-parameter values is noted as a coincidental order-of-magnitude proximity, not treated as validation — tidal-strength ratio and spin parameter are different quantities (§3, corrected).
...
- **The "nested relay" (successive transmission through the hierarchy) is a candidate interpretation of the computed tidal ratios, not proven by them — an independent review correctly caught this document overstating it earlier. Similar ratio magnitudes across adjacent levels are consistent with a relay but do not rule out each level's rotation arising largely from independent local formation dynamics.**
- **The proximity between computed tidal ratios and real galaxy spin-parameter values (§3) is an order-of-magnitude coincidence worth noting, not a validation — a tidal-strength ratio and a spin parameter (`λ`) are different quantities with different definitions, and matching magnitude alone does not establish either quantity predicts the other.**
- The field this document's tidal tensor is built from is a *candidate* derived field (`Internal_Proofs/08`, corrected after external review) — §1's tidal-tensor form is unaffected by the specific correction, but the field's overall status should not be read as more settled than that.
