(() => {
  ...
  const FLIP_MS=95;                 // fast nerve-reactive layer
  const NERVE_PER_SUPERVISOR=3;     // 3:1 fast nerve : oversight boundary
  const FULL_PIPELINE_FLIPS=6;       // 6:1 full oversight/override chronology
  ...
  // One supervisory boundary is a paired exchange, not two separate events:
  // NEW oversight goes UP while the LAST completed decision/action comes DOWN.
  // The down packet is therefore chronologically one supervisory boundary behind.
  function supervisoryExchange(side,m,label){
    ...
    // What just went up is staged to come down at the NEXT boundary.
  ...
  async function nerveFlip(side,m,i){
    ...
    // Three-winding nerve reaction is the fast layer. It keeps moving/reacting
    // between the slower supervisory exchanges.
  ...
  async function touch(side,m){
    ...
    // Pointer contact starts the local nerve action. The supervisory layer does
    // not stop that fast reaction; it samples every third flip.
    ...
    // Final contact is the actuator boundary. The move executor is called only
    // after the six-flip reel has carried the staggered up/down pipeline.
