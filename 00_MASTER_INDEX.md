...
Updated 44 (renumbered from 42 — collision with PR #6's parallel branch) Keplerian-limit derivation handoff (closes the inverse-square/Kepler gap underneath Updated 38-41 and D-413; two math errors caught by external review and corrected — see the document's own notice): `UPDATED_44_KEPLERIAN_LIMIT_DERIVATION.md`; full math in `Internal_Proofs/08_Keplerian_Limit_Derivation.md`.
...
| A-105 | Restoring Response | The field's reaction to gradient imbalance: R_OW = −A(∇ψ); far-field inverse-square limit now derived, not sketched — see Updated 44. | YELLOW |
...
| C-306 | Torque | Rotational preference from off-center displacement; τ=0 for the derived central-force case (Updated 44), closing that application. | GREEN |
| C-307 | Angular Momentum | Product of angular velocity ω and rotational inertia I; central-force conservation now derives Kepler's Second Law (Updated 44). | GREEN |
...
| 12 | Gravity at Micro Scale — Ever-Changing Gradient Field | Gradient response (A-105), not force-carrier exchange; null-particle structure; far-field inverse-square limit and Kepler's laws now derived, not sketched (Updated 44) — equivalence-principle bridge, general operator form, and the a-vs-alpha relationship (found during external review) remain open. | GREEN (struct.) / YELLOW (full deriv.) |
...
A-106, A-107 (both now closer after the FIXED versions), Higgs (Ch15), Gravity (Ch12) — far-field inverse-square limit and Kepler's three laws now derived (Updated 44, corrected after external review found real errors in the original G_eff formula and shell-theorem claim); the general nonlinear operator, `b`'s origin, the a-vs-alpha relationship, and the equivalence-principle bridge remain owed — Photon (Ch7), most Book 1 chapters.
