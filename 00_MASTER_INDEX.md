...
| G-721 | Mirrored Alphabet Rabbit-Hop Coordinate Algorithm | Primitive state is the triple (n,k,s) -- identity, reference displacement, local polarity; derived p=n+k, r=2p+s (s∈{-1,0,+1}) reproduces the original ±(n,2n,2n+1) packet and a later two-family proposal as special cases, brute-force verified collision-free; T_a/P reference-shift and polarity operators proven to compose and commute; distinguishes half-mirror reversal from round-trip orientation-exchange; carries a boundary-noted 12-tone coordinate extension separate from E-510. | YELLOW |
