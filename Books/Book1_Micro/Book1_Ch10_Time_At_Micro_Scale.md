...
**Checked directly, not left as a pointer (C-313's exact general-gamma solution):** does `γ` itself — this chapter's own candidate mechanism, "resistance to moving through the medium" — carry a k-dependence that could couple to a mode's translational speed (which enters via its wave-packet's central k, through group velocity)? Solved exactly: no. `γ` produces a flat, k-independent decay envelope (`|z|=√(1-γ)` exactly, in the propagating branch) with no curvature for speed to couple through. It also produces a real, new, previously-unstated structural fact — a long-wavelength cutoff, `x_crit=(2-γ)-2√(1-γ)`, below which modes don't propagate at all for any γ>0 — but that is a statement about which wavelengths propagate, not about how fast a translating mode's internal clock runs. This is a genuine, checked, negative result for the specific "γ directly causes dilation" version of this chapter's hypothesis, not a hand-wave: see C-313 for the full derivation. The dispersion relation's own curvature at large k (it saturates near the lattice zone edge rather than staying linear) remains the more promising, still-unexplored lever — flagged, not attempted.
