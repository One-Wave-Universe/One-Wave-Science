# ONE-WAVE FRAMEWORK
## Book 1 — Micro
## Chapter 7: The Photon Measurement and the Propagating Light Mode
...
Dependencies: C-309 Friction Limit, C-311 Electric-Magnetic Duality, E-528 Static Redshift Transport,
              A-08 Oscillation, A-10 Persistent Mode, B-06b Four Views, D-02 Resonant Mode
Status: GREEN (structure) / YELLOW (full derivation)

---

## Gray — Standard Model Reference

The photon is the force-carrier particle of electromagnetism.
Mass: 0. Charge: 0. Spin: 1.
The photon travels at c = 299,792,458 m/s in vacuum.
The photon exhibits wave-particle duality — it behaves as a wave (interference, diffraction)
and as a particle (photoelectric effect, Compton scattering).
Photon energy: E = hf = hbar * omega.
Photon momentum: p = h/lambda = hbar * k.
The photon is its own antiparticle.
In quantum field theory, the photon is an excitation of the electromagnetic field.

Standard Model strength: QED (quantum electrodynamics) is the most precisely tested theory
in physics. The photon mediates all electromagnetic interactions.
Standard Model limitation: wave-particle duality remains philosophically unresolved.
Why light travels at exactly c is explained by Maxwell's equations but not derived
from a deeper principle. The photon mass being exactly zero is assumed, not derived.
...
\omega(k	o0)	o0,
\]
and has no finite-energy localized rest solution.
...
Registered photon energy (light-wave transfer):
E = hbar * omega

Registered light-wave momentum transfer:
p = hbar * k = E / c

Electric and magnetic components (C-311):
E_vec ~ nabla P_psi
B_vec ~ nabla x P_psi
|E_vec| = c * |B_vec|  (standard relation — derivation from lattice geometry deferred)

Dispersion relation:
omega = c * k  (massless, traveling at friction limit)

Photoelectric threshold:
E_photon >= DeltaE_shell
hbar * omega >= E_{n+1} - E_n
hbar * omega >= hbar * omega_0
omega >= omega_0  (threshold frequency)

Light-wave spin-1 measurement (2*pi closure):
psi(theta + 2*pi) = psi(theta)  (returns to itself after one rotation)
...
One-Wave: momentum transfer is lattice pressure pulse transfer between
traveling mode and shell mode. Formal derivation deferred.
...
Prediction: the sharp threshold frequency in the photoelectric effect
corresponds to the minimum energy required to transition an electron
to the next harmonic shell.

5. No wave-particle duality needed.
Prediction: double-slit interference is a traveling pressure wave interfering with itself
(F-05 Interference at two boundaries simultaneously).
The measurement at the detector is a phase sample, not a collapse.

6. The Propagating Light Mode has zero **rest Mass Effect** only if its branch is genuinely gapless and lacks a localized rest solution.
A reduced group speed in matter or in a nonuniform field does not, by itself, imply a nonzero rest Mass Effect for the light-wave branch. Standard experimental bounds conventionally reported as photon rest-mass bounds remain the Gray comparison target.
...
- Photon-registration energy quantization from harmonic shell condition not yet derived
...
Derive full electromagnetic wave equations from lattice pressure oscillation.
Show |E_vec| = c * |B_vec| from lattice geometry.
Formalize Compton scattering as lattice momentum transfer.
Derive spin-1 formally from traveling mode 2*pi closure.
Connect photon-registration energy quantization to D-05 harmonic shell resonance.
...
Wave-particle duality has been a source of philosophical confusion for a century.
Is light a wave or a particle? The answer in quantum mechanics is: both, somehow.

In One-Wave, the question dissolves.
Light is a traveling pressure oscillation in the superfluid lattice.
It is not a wave or a particle. It is a mode of the field.
